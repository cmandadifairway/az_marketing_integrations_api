resource "azurerm_resource_group" "ffadmin" {
...
resource "azurerm_function_app" "ffadmin" {
  name                       = "fn-fim-${local.settings.subscipt}-${local.settings.environment}-${local.settings.locabbrev}-${local.settings.service}"
  location                   = local.settings.location
  resource_group_name        = azurerm_resource_group.ffadmin.name
  app_service_plan_id        = azurerm_app_service_plan.ffadmin.id
  storage_account_name       = azurerm_storage_account.ffadmin.name
  ...
  app_settings = {
    ...
    "APPINSIGHTS_INSTRUMENTATIONKEY"        = azurerm_application_insights.ffadmin.instrumentation_key
