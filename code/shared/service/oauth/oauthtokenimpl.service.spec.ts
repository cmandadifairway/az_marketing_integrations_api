...
describe("test getting oauth token for APIM gateway", () => {
	...
	beforeEach(() => {
		...
		jest.spyOn(CustomLoggerImpl.prototype, "error").mockImplementation(() => Promise.
	resolve());
		jest.spyOn(CustomLoggerImpl.prototype, "critical").mockImplementation(() => 
	Promise.resolve());
	...
	test(" getAPIMToken success flow for local with DefaultAzureCredential ", async () => {
		...
		jest.spyOn(container.get<AppConfigService>(TYPES.AppConfigService)
			, "getConfiguration").mockImplementation(() => Promise.resolve("123"));
		//@ts-ignore
