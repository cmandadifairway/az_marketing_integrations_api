...
export class KeyVaultServiceImpl implements KeyVaultService {
  ...
  this.logger.info(`getSecretValue Method Initiated with secretName ${secretName}`);
   ...
   if(!secretValue){
    try {
      ...
      this.logger.info(`secretValue recieved from keyVault for secretKey :: ${secretName}`);
      secretValue=secretObj.value;
    } catch (error) {
      this.baseErrorHandler.handleError(error,"error in KeyVaultServiceImpl.getSecretValue while getting secret: ");
   }
