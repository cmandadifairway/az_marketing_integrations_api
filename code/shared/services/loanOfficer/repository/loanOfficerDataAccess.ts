import { LoanOfficerByName } from "../../../model/loanOfficerByNameResponse";
...
export class LoanOfficerDataAccess extends DataAccessBase implements LoanOfficerDataService {
    async getLoanOfficer(query: object, throwError: boolean): Promise<LoanOfficerResponse> {
        ...
        try {
            ...
            this.customLogger.error("Error while trying to get LO by " + JSON.stringify(query), error);
            if (throwError) {
                throw error;
            }
        }
    ...
    async getLoanOfficerByName(query: object, throwError: boolean): Promise<LoanOfficerByNameResponse> {
        ...
        try {
            ...
            this.customLogger.error("Error while trying to get LOs by name", error);
            if (throwError) {
                throw error;
            }
        }
    ...
    async updateLoanOfficer(query: object, updateQuery: object): Promise<Response> {
        ...
        try {
            ...
            this.customLogger.error(`Error while updating loan officer ${queryString} in db`, error);
            throw error;
        }
        return response;
    }
